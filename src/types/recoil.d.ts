...
declare namespace __Recoil {
  ...
  type AtomFamilyOptions<T, P extends AtomFamilyParameter> = Readonly<
    AtomOptions<T> & {
      default: RecoilValue<T> | Promise<T> | T | ((param: any /*FIXME*/) => T | RecoilValue<T> | Promise<T>) | any; // FIXME
    }
  >;

  function atomFamily<T, P extends AtomFamilyParameter>(options: AtomFamilyOptions<T, P>): (param: P) => RecoilState<T>;
}
